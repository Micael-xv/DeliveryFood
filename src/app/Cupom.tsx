...
export default function CupomScreen() {
  ...
  const fetchCupons = async () => {
    try {
      const response = await axios.get('http://192.168.38.192:3333/cupoms');
  ...
  const handleSave = async () => {
    try {
      ...
      if (currentCupom.id) {
        ...
        response = await axios.post(`http://192.168.38.192:3333/cupoms/persist/${currentCupom.id}`, currentCupom);
      } else {
        console.log('Criando cupom:', currentCupom);
        response = await axios.post('http://192.168.38.192:3333/cupoms/persist', currentCupom);
      }
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              const response = await axios.post('http://192.168.38.192:3333/cupoms/destroy', { id: cupom.id });
              if (response.data.success) {
